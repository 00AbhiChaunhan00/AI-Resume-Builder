...
// this function is used to send the data to api on strapi tool
  const onCreate=async()=>{
      setloading(true)  // while calling loading become true so loading button should appear
    const uuid=uuidv4() // uuid function calls through import and get the new id
        const data={         // to send this data to the api thorough which we can access 
          data:{
            title:resumeTitle,
            resumeId:uuid,
        ...
        console.log("sending the data",data)
         ...
         .then(res=>{
          ...
          if(res){
            setloading(false)   // this is for the loaing button after response make it false 
         ...
         },(error)=>{        // we write this here beacuse if the api fails response tha this callback functio execute
          setloading(false);
 ...
 from-pink-200 via-purple-200 to-blue-200
      border-primary
      border items-center flex justify-center bg-secondary rounded-lg h-[300px] hover:scale-105 transition-all hover:shadow-md cursor-pointer' onClick={()=>setopenDialog(true)} >
...
<Dialog open={Opendialog} > 
  <DialogContent>
    <DialogHeader>
      ...
      <DialogDescription>
        <span className='flex justify-start text-[15px]'>Add a title for  your resume
        </span>
      <Input className="mt-3" placeholder="Ex. Full stack developer" 
      ...
      <div className='flex justify-end gap-5'>
        <Button onClick={()=>setopenDialog(false)} variant="ghost">Cancel</Button>
         ...
         disabled= // this disabled keyword is built-in
         {!resumeTitle || loading}
         onClick={()=>onCreate()}>{loading ? <Loader2 className='animate-spin' />:"Create"}
         </Button>
      </div>
    </DialogHeader>
  </DialogContent>
