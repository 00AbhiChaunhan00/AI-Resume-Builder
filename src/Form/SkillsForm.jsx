...
//   ResumeInfo&&setSkillsList(ResumeInfo?.skills)  // here the experance is the data from api strapi
// },[])
      ...
      {skillsList.map((item,index)=>{
           return <div >
            <div className='flex justify-between border mb-2 rounded-lg p-3'>
              <div>
                <label >Skills</label>
                <Input defaultValue={item.name} onChange={(e)=>handleInput(index,'name',e.target.value)}/>
              </div>
               <Rating rating={item.rating} setRating={(v)=>handleInput(index,'rating',v)}/>
            </div>
           </div>
      })}
       <div className='flex justify-between'>
      <div className='flex gap-2'>
        <Button  style={{backgroundColor:'#F87171'}} onClick={RemoveSkills} >- Remove</Button>
        <Button onClick={AddNewSkills} className='text-primary' variant='outline'> + Add Experrience</Button>
      </div>
